...
type SunglassFormValues = z.infer<typeof sunglassSchema>;

interface AddSunglassFormProps {
  onProductUpdated: () => void;
  existingProduct: Sunglass | null;
}

export function AddSunglassForm({ onProductUpdated, existingProduct }: AddSunglassFormProps) {
  const { toast } = useToast();
  const [isSubmitting, setIsSubmitting] = useState(false);
  const isEditMode = !!existingProduct;

  const form = useForm<SunglassFormValues>({
    resolver: zodResolver(sunglassSchema),
    defaultValues: {
        name: '',
        description: '',
        mrp: 0,
  ...
  async function onSubmit(values: SunglassFormValues) {
    ...
    try {
      ...
      if (isEditMode) {
        const { error } = await supabase.from('sunglasses').update({
            ...valuesToInsert,
        ...
        toast({ title: 'Product Updated', description: `${values.name} has been updated.` });
         ...
         const { error } = await supabase.from('sunglasses').insert({
            ...valuesToInsert,
            image_urls: finalImageUrls,
        });
        if (error) throw error;
