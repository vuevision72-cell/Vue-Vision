...
interface SunglassesTableProps {
  ...
  onEditProduct: (product: Sunglass) => void;
  filters: {
    searchTerm: string;
    gender: string;
    frameType: string;
...
export function SunglassesTable({ dataVersion, onProductDeleted, onEditProduct, filters }: SunglassesTableProps) {
  const [allSunglasses, setAllSunglasses] = useState<Sunglass[]>([]);
  const [filteredSunglasses, setFilteredSunglasses] = useState<Sunglass[]>([]);
  ...
  useEffect(() => {
    ...
    const filteredData = allSunglasses.filter(item => {
    ...
    setFilteredSunglasses(filteredData);
  }, [filters, allSunglasses]);
  ...
  const ActionsMenu = ({ product }: { product: Sunglass }) => (
     <AlertDialog>
        <DropdownMenu>
    ...
    <div className="rounded-md border">
        ...
        <div className="hidden md:block">
            ...
            <TableBody>
                ...
                {!isLoading && filteredSunglasses.length === 0 && (
                    <TableRow>
                ...
                {!isLoading && filteredSunglasses.map((product) => (
