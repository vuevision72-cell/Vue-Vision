...
interface EyeglassesTableProps {
  dataVersion: number;
  onProductDeleted: () => void;
  onEditProduct: (product: Eyeglass) => void;
  filters: {
    searchTerm: string;
    gender: string;
    frameType: string;
...
export function EyeglassesTable({ dataVersion, onProductDeleted, onEditProduct, filters }: EyeglassesTableProps) {
  const [allEyeglasses, setAllEyeglasses] = useState<Eyeglass[]>([]);
  const [filteredEyeglasses, setFilteredEyeglasses] = useState<Eyeglass[]>([]);
  ...
  useEffect(() => {
    const fetchEyeglasses = async () => {
      ...
      if (error) {
        ...
        setAllEyeglasses(data || []);
      }
      setIsLoading(false);
    };

    fetchEyeglasses();
  }, [dataVersion]); // Refetch when dataVersion changes

  useEffect(() => {
    const lowercasedFilter = filters.searchTerm.toLowerCase();
    const filteredData = allEyeglasses.filter(item => {
    ...
    setFilteredEyeglasses(filteredData);
  }, [filters, allEyeglasses]);
  ...
  const ActionsMenu = ({ product }: { product: Eyeglass }) => (
    <AlertDialog>
        <DropdownMenu>
    ...
    <div className="rounded-md border">
        ...
        <div className="hidden md:block">
            ...
            <TableBody>
                ...
                {!isLoading && filteredEyeglasses.length === 0 && (
                    <TableRow>
                ...
                {!isLoading && filteredEyeglasses.map((product) => (
